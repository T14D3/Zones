...
public class RegionKeyArgument implements CustomArgumentType.Converted<Region, String> {
    ...
    public static Component regionInfo(Map.Entry<String, Region> entry) {
        ...
        comp = comp.append(mm.deserialize("<light_purple>Name: </light_purple>" + messages.get("region.info.name") + " ", parsed("name", entry.getValue().getName())));
        if (entry.getValue().getParent() != null) {
            comp = comp.append(mm.deserialize(messages.get("region.info.parent") + " ", parsed("parent", entry.getValue().getParent())));
        }
        comp = comp.append(mm.deserialize("<green>(</green>" + messages.get("region.info.min") + " - ", parsed("min", entry.getValue().getMinString())));
        comp = comp.append(mm.deserialize(messages.get("region.info.max") + "<green>)</green>", parsed("max", entry.getValue().getMaxString())));
